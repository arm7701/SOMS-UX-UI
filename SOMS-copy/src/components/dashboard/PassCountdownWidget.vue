...
<template>
  <div class="w-full h-full flex flex-col flex-1 min-h-0 bg-transparent overflow-hidden">
    <!-- Top Header Bar -->
    <div class="px-4 py-3 bg-[#0d1117] border-b border-space-700/80 flex flex-wrap items-center justify-between gap-2.5">
      <div class="flex items-center gap-2.5">
        <div class="w-8 h-8 rounded-lg bg-sky-950/40 border border-sky-700/60 flex items-center justify-center text-sky-400">
          <Clock class="w-4 h-4 animate-pulse" />
        </div>
        <div>
          <h3 class="text-sm font-bold font-prompt text-white tracking-wide flex items-center gap-2">
            Pass Countdown
            <span
              v-if="activeOrNextPass?.isInContact"
              class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-950/90 border border-emerald-500 text-emerald-300 animate-pulse"
            >
            ...
            <span
              v-else
              class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-zinc-800 border border-zinc-600 text-slate-200 font-mono"
            >
              UPCOMING
            </span>
          </h3>
          <p class="text-xs text-slate-200 font-medium font-prompt mt-0.5">
            เวลานับถอยหลังรอบพาสและการติดต่อสื่อสารดาวเทียม
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2 flex-wrap">
        ...
        <select
          ...
          class="px-3 py-1.5 text-xs sm:text-sm rounded-lg border border-space-600 bg-[#090b0f] text-slate-100 font-prompt font-medium focus:outline-none focus:border-zinc-400"
        >
    ...
    <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-space-700">
        <div>
          <div class="flex items-center gap-2 flex-wrap">
            ...
            <span class="text-base font-extrabold font-prompt text-white">
              {{ activeOrNextPass?.satName || 'NAPA-2 N' }}
            </span>
          </div>
          <p class="text-xs sm:text-sm text-slate-200 font-prompt font-medium mt-1 flex items-center gap-1.5">
        ...
        <div class="flex items-center gap-2">
          <div class="px-3.5 py-2 rounded-xl bg-[#0d1117] border border-space-600 text-right">
            <p class="text-xs text-slate-300 font-prompt font-semibold">มุมยกสูงสุด (Max El)</p>
            <p class="text-base font-black font-mono text-amber-300">
      ...
      <div class="py-2 text-center">
        <p class="text-xs sm:text-sm uppercase tracking-widest text-slate-200 font-bold mb-2.5 font-prompt">
        ...
        <div class="inline-flex items-center justify-center gap-2 sm:gap-4 bg-[#090b0f] px-7 py-4 rounded-2xl border border-space-600 shadow-inner">
          <div class="text-center">
            <div class="text-3xl sm:text-5xl lg:text-6xl font-mono font-black tracking-tight text-white">
              {{ countdown.hours }}
            </div>
            <span class="text-xs uppercase tracking-wider text-slate-200 font-bold font-prompt">ชั่วโมง</span>
          </div>
          <span class="text-3xl sm:text-4xl font-mono font-black text-slate-400 -mt-4">:</span>
          <div class="text-center">
            <div class="text-3xl sm:text-5xl lg:text-6xl font-mono font-black tracking-tight text-cyan-300">
              {{ countdown.minutes }}
            </div>
            <span class="text-xs uppercase tracking-wider text-slate-200 font-bold font-prompt">นาที</span>
          </div>
          <span class="text-3xl sm:text-4xl font-mono font-black text-slate-400 -mt-4">:</span>
          <div class="text-center">
            <div class="text-3xl sm:text-5xl lg:text-6xl font-mono font-black tracking-tight text-emerald-300">
              {{ countdown.seconds }}
            </div>
            <span class="text-xs uppercase tracking-wider text-slate-200 font-bold font-prompt">วินาที</span>
          </div>
        </div>

        <!-- In-Pass Progress Bar (ถ้าอยู่ในรอบพาส) -->
        <div v-if="activeOrNextPass?.isInContact" class="mt-4 max-w-md mx-auto space-y-1.5">
          <div class="h-2.5 w-full rounded-full bg-[#141923] overflow-hidden border border-space-600">
            <div
      ...
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-2 border-t border-space-700 text-xs font-prompt">
        <div class="p-2.5 rounded-xl bg-[#0d1117] border border-space-600">
          <span class="text-slate-300 text-xs block font-semibold">เวลา AOS (UTC)</span>
          <p class="font-mono text-white font-bold text-sm mt-0.5">
            {{ activeOrNextPass?.aos_time_utc || '—' }}
          </p>
        </div>
        <div class="p-2.5 rounded-xl bg-[#0d1117] border border-space-600">
          <span class="text-slate-300 text-xs block font-semibold">เวลา LOS (UTC)</span>
          <p class="font-mono text-white font-bold text-sm mt-0.5">
            {{ activeOrNextPass?.los_time_utc || '—' }}
          </p>
        </div>
        <div class="p-2.5 rounded-xl bg-[#0d1117] border border-space-600">
          <span class="text-slate-300 text-xs block font-semibold">เวลาท้องถิ่น (Local)</span>
          <p class="font-mono text-emerald-300 font-bold text-sm mt-0.5">
            {{ activeOrNextPass?.aos_time_local || '—' }}
          </p>
        </div>
        <div class="p-2.5 rounded-xl bg-[#0d1117] border border-space-600">
          <span class="text-slate-300 text-xs block font-semibold">ระยะเวลาพาส</span>
          <p class="font-mono text-white font-bold text-sm mt-0.5">
      ...
      <div class="p-3 rounded-xl bg-[#090b0f] border border-space-600 text-xs font-prompt">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
          <div class="bg-[#0d1117] p-2 rounded-lg border border-space-600">
            <span class="text-slate-300 text-xs block font-prompt font-semibold">UHF Uplink</span>
            <span class="text-white font-bold text-sm">437.525 MHz</span>
          </div>
          <div class="bg-[#0d1117] p-2 rounded-lg border border-space-600">
            <span class="text-slate-300 text-xs block font-prompt font-semibold">S-Band Downlink</span>
            <span class="text-cyan-300 font-bold text-sm">2245.00 MHz</span>
          </div>
          <div class="bg-[#0d1117] p-2 rounded-lg border border-space-600">
            <span class="text-slate-300 text-xs block font-prompt font-semibold">X-Band Payload</span>
            <span class="text-amber-300 font-bold text-sm">8150.00 MHz</span>
          </div>
          <div class="bg-[#0d1117] p-2 rounded-lg border border-space-600">
            <span class="text-slate-300 text-xs block font-prompt font-semibold">Link SNR</span>
      ...
      <div class="space-y-2 pt-1 border-t border-space-700">
        ...
        <div class="space-y-1.5 overflow-y-auto max-h-[130px] pr-1">
          <div
            ...
            :class="p.id === activeOrNextPass?.id ? 'bg-cyan-950/60 border border-cyan-500 text-white font-semibold' : 'bg-[#0d1117] border border-space-600 text-slate-200'"
          >
            <div class="flex items-center gap-2.5">
            ...
            <div class="flex items-center gap-2.5">
              ...
              <span
                ...
                :class="p.maxElevation >= 60 ? 'bg-emerald-950 text-emerald-300 border border-emerald-600' : p.maxElevation >= 30 ? 'bg-amber-950 text-amber-300 border border-amber-600' : 'bg-zinc-800 text-slate-200 border border-zinc-600'"
              >
                {{ p.maxElevation }}° El
              </span>
            </div>
      ...
      <div class="flex items-center justify-between gap-3 pt-2 border-t border-space-700/60">
        <button
          type="button"
        ...
        <button
          type="button"
          class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-700 hover:bg-zinc-600 text-white text-xs font-medium font-prompt border border-zinc-500/60 shadow-sm transition-all cursor-pointer"
