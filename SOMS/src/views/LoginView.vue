...
<template>
  <div class="relative min-h-screen w-full flex items-center justify-center p-4 sm:p-6 overflow-hidden bg-[#030816] text-slate-100 select-none">
    ...
    <div
      :class="[
        'fixed top-5 left-5 z-20 hidden sm:flex items-center gap-3 px-3.5 py-2.5 rounded-2xl bg-[#0b1f38]/90 border border-sky-400/40 backdrop-blur-md shadow-xl select-none transition-all duration-500 cursor-pointer',
        isZoomingIn ? 'opacity-0 -translate-y-4 pointer-events-none' : 'opacity-100 translate-y-0'
      ]"
      @dblclick="autofillDev"
      title="Double-click for developer testing"
    >
      <img
        src="/src/assets/soms-logo.png"
        alt="SOMS Emblem"
        class="h-10 w-auto object-contain filter drop-shadow-[0_0_12px_rgba(56,189,248,0.6)]"
      />
        ...
        <div class="text-center mb-7">
          ...
          <div>
            <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-sky-500/15 border border-sky-400/40 text-cyan-300 text-xs font-bold tracking-wider uppercase mb-2 shadow-xs">
              ...
              <span>Royal Thai Air Force</span>
            </div>
            <h1 class="text-2xl sm:text-[28px] font-extrabold text-white tracking-[0.02em] leading-snug drop-shadow-sm font-prompt">
              ระบบปฏิบัติการดาวเทียม
            </h1>
            <p class="text-sm sm:text-[15px] uppercase tracking-[0.16em] text-cyan-300 font-bold font-orbitron mt-1">
              Satellite Operations Management System (SOMS)
            </p>
          </div>
        </div>
    ...
    >
      <div class="text-center space-y-3 animate-pulse">
        ...
        <p class="text-2xl sm:text-3xl font-black text-white tracking-[0.2em] drop-shadow-[0_0_20px_rgba(56,189,248,0.9)]">
          INITIALIZING SOMS DASHBOARD
        </p>
      </div>
    </div>

    <!-- 7. Interactive Globe View Toggle Button (ปุ่มสลับมุมมองชมลูกโลกเต็มจอ) -->
    <button
      v-if="!isZoomingIn"
      type="button"
      ...
      :title="cardMinimized ? 'แสดงกล่องเข้าสู่ระบบ' : 'ซ่อนหน้าต่างเพื่อชมลูกโลกกราฟฟิกเต็มจอ'"
    >
      <Globe class="w-4 h-4 sm:w-5 sm:h-5 text-cyan-400 group-hover:rotate-45 transition-transform duration-300" />
      <span>{{ cardMinimized ? 'แสดงหน้าต่างเข้าสู่ระบบ' : 'ชมลูกโลกเต็มจอ' }}</span>
    </button>
  </div>
</template>
