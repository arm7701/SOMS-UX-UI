    ...
    <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-5">
      ...
      <div class="space-y-2.5 flex-1 min-w-0">
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            :class="appStore.audioAlertsEnabled
              ? 'bg-sky-950/70 border-sky-400 text-cyan-300 shadow-xs'
              : 'bg-space-850 border-space-700 text-sky-200 hover:text-white'"
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 pt-1 text-xs">
          <div class="bg-[#1a4175]/90 backdrop-blur-xs p-3 rounded-xl border border-sky-600/40">
            <span class="text-sky-200 block text-xs font-prompt font-medium">ช่วงเวลาผ่าน (เวลาไทย):</span>
          ...
          <div class="bg-[#1a4175]/90 backdrop-blur-xs p-3 rounded-xl border border-sky-600/40">
            <span class="text-sky-200 block text-xs font-prompt font-medium">เวลาสากล (UTC):</span>
            <span class="font-mono font-bold text-white text-xs sm:text-sm mt-0.5 block">
          ...
          <div class="bg-[#1a4175]/90 backdrop-blur-xs p-3 rounded-xl border border-sky-600/40">
            <span class="text-sky-200 block text-xs font-prompt font-medium">มุมยกสูงสุด (Max El):</span>
          ...
          <div class="bg-[#1a4175]/90 backdrop-blur-xs p-3 rounded-xl border border-sky-600/40">
            <span class="text-sky-200 block text-xs font-prompt font-medium">ระยะเวลาผ่านสถานี:</span>
            <span class="font-mono font-bold text-white text-xs sm:text-sm mt-0.5 block">
      ...
      <div class="flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end justify-between gap-4 pt-2 lg:pt-0 border-t lg:border-t-0 border-sky-600/30">
        <div class="text-left lg:text-right">
          <span class="text-xs font-semibold text-sky-200 uppercase tracking-wide block font-prompt">
            {{ countdown.label }}
          </span>

          <div class="flex items-center gap-2 mt-1.5 font-mono">
            <div class="bg-[#0e2b50] px-3 py-2 rounded-xl border border-sky-500/50 text-center min-w-[56px] shadow-md">
              <span class="text-3xl sm:text-4xl font-bold text-white tracking-tight">{{ countdown.hours }}</span>
              <span class="block text-xs text-sky-300 font-medium uppercase mt-0.5">ชม.</span>
            </div>
            ...
            <div class="bg-[#0e2b50] px-3 py-2 rounded-xl border border-sky-500/50 text-center min-w-[56px] shadow-md">
              ...
              <span class="block text-xs text-sky-300 font-medium uppercase mt-0.5">นาที</span>
            </div>
            <span class="text-2xl font-bold text-cyan-400 animate-pulse">:</span>
            <div class="bg-[#0e2b50] px-3 py-2 rounded-xl border border-sky-500/50 text-center min-w-[56px] shadow-md">
              <span class="text-3xl sm:text-4xl font-bold text-white tracking-tight">{{ countdown.seconds }}</span>
              <span class="block text-xs text-sky-300 font-medium uppercase mt-0.5">วินาที</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex items-center gap-2 w-full sm:w-auto">
          <button
            type="button"
            class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl border border-sky-500/40 bg-space-800 hover:bg-space-700 text-white text-xs sm:text-sm font-bold shadow-xs transition-colors cursor-pointer font-prompt"
          ...
          <button
            type="button"
            class="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-400 hover:to-blue-500 text-white border border-sky-400/50 text-xs sm:text-sm font-bold shadow-md shadow-sky-500/30 transition-all cursor-pointer font-prompt"
