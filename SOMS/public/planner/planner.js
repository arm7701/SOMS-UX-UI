...
const DEFAULT_CHECKED = new Set(['48963','58016','40697','42063','39084']);
...
function drawTimeline() {
  ...
  while (dayT <= tMax) {
    ...
    ctx.fillStyle='#93c5fd'; ctx.font='9px "Prompt", monospace'; ctx.textAlign='center';
  ...
  satIds.forEach((id, ri) => {
    ...
    // Row bg
    ctx.fillStyle = ri%2===0 ? '#0d284f' : '#10325c';
...
function renderTargetList() {
  ...
  if (!savedTargets.length) {
    el.innerHTML='<div style="font-size:10px;color:#bae6fd;padding:4px 0">ยังไม่มี target — คลิกแผนที่แล้วกด Save</div>';
    return;
  }
...
function buildSatList() {
  ...
  getOrderedSatIds().forEach(id => {
    ...
    const stHtml  = hasTLE ? `<span style="color:#4ade80;font-size:9.5px;font-weight:600">TLE ✓</span>` : `<span style="color:#93c5fd;font-size:9.5px">no TLE</span>`;
    ...
    row.innerHTML = `
      ...
      <input type="checkbox" id="chk_${id}" ${checked?'checked':''} onclick="event.stopPropagation();updateSpecBox()" style="width:14px;height:14px;cursor:pointer;flex-shrink:0;accent-color:#0284c7">
      <div class="sat-dot" style="background:${s.color}"></div>
      <span class="sat-name" style="color:${s.color};font-size:11.5px;font-weight:600;flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${s.name}</span>
...
function initMap() {
  ...
  window._curLayer = window._layers.dark;
  window._curLayer.addTo(map);
  ...
  _lsWrap.innerHTML = '<button onclick="setLayer(\'dark\',this)" class="ls-btn ls-act">🌑 Dark</button><button onclick="setLayer(\'sat\',this)" class="ls-btn">🛰 Satellite</button><button onclick="setLayer(\'hybrid\',this)" class="ls-btn">🌐 Hybrid</button><button onclick="setLayer(\'street\',this)" class="ls-btn">🗺 Street</button>';
  document.getElementById('mwrap').appendChild(_lsWrap);

  // Restore saved map layer
  try{
    const saved=localStorage.getItem('map_layer');
    if(saved&&window._layers[saved]&&saved!=='dark'){
      map.removeLayer(window._curLayer);
      window._curLayer=window._layers[saved];
      window._curLayer.addTo(map);
      setTimeout(()=>{ document.querySelectorAll('.ls-btn').forEach(b=>{ if(b.onclick&&b.onclick.toString().includes("'"+saved+"'")) b.classList.add('ls-act'); else b.classList.remove('ls-act'); }); },100);
    }
  }catch(e){}
  map.on('click', e => {
...
function renderTbl(ps) {
  ...
  if (!ps.length) {
    tb.innerHTML='<tr><td colspan="16" style="text-align:center;padding:24px;color:#bae6fd">ไม่พบ pass ตาม filter ที่เลือก</td></tr>';
    return;
  }
  tb.innerHTML = ps.map((p,i) => {
    const realIdx = allPasses.indexOf(p);
    const dm=Math.floor(p.dur/60), ds=Math.round(p.dur%60);
    const elC  = p.maxEl>=45?'#38bdf8':p.maxEl>=30?'#4ade80':p.maxEl>=15?'#fde047':'#93c5fd';
    ...
    // Cloud badge
    let cloudBadge = '<span style="color:#93c5fd">—</span>';
    ...
    return `<tr onclick="showPass(${realIdx})" id="r${realIdx}">
      <td style="color:#38bdf8;font-weight:600">${i+1}</td>
      <td><span style="color:${p.satColor};font-weight:600;font-size:10px">${p.satName}</span></td>
      ...
      <td style="color:#bae6fd">${p.rollFeasible?along+' km':'—'}</td>
...
function renderStats() {
  ...
  document.getElementById('sbody').innerHTML =
    ...
    `<div class="srow"><span class="sl">Good imaging</span><span class="sv" style="color:${good?'#4ade80':'#93c5fd'}">${good}</span></div>`+
    `<div class="srow"><span class="sl">Feasible</span><span class="sv">${feas}</span></div>`+
    `<div class="srow"><span class="sl">Best elev</span><span class="sv" style="color:#38bdf8">${best.maxEl.toFixed(1)}° (${best.satName})</span></div>`+
    `<div class="srow"><span class="sl">By sat</span><span class="sv" style="font-size:9.5px;color:#bae6fd">${satline}</span></div>`;
...
function showPass(idx) {
  ...
  if (p.rollFeasible) {
    ...
    pLayers.push(poly);
    pLayers.push(L.circleMarker(startM,{radius:5,color,fillColor:'#0a2342',fillOpacity:1,weight:2}).addTo(map).bindTooltip('▶ IMG START '+fmtTime(p.imageStartDt)));
    ...
    const tsIcon=L.divIcon({html:`<div style="background:${color};color:#0a2342;font-family:'Prompt',monospace;font-size:9.5px;font-weight:700;padding:2px 6px;border-radius:4px;white-space:nowrap;margin-top:4px;box-shadow:0 2px 6px rgba(0,0,0,0.4)">TS ${fmtTime(p.maxElDt)}</div>`,className:'',iconAnchor:[0,0]});
